...
let data = [
   ...
   let div = document.querySelector("#box_cards");////находим узел по id;
    const showCard  = (arr) =>{

        div.innerHTML = " ";//обновление экрана перед отрисовкой.
        arr.forEach(element => {

            console.log(element.symbol + element.title + element.keywords);
            let Card = document.createElement("div");//создаем узел с <div></div>;
              Card.classList.add("styleCard");// задаем класс узлу.
              
              Card.innerHTML += `<div class = "elem"><div class = "smile">${element.symbol}</div><br> <div class = "text1">${element.title}</div> <br> <div class = "text2"> ${element.keywords} </div></div>`;//добавляем в узел контент.
              div.append(Card);// добавляем в body узел </div>

        });
    };
    
    showCard(data);
    let search = document.querySelector('#text');//находим узел по id;


   
    let emogySearch = (arr) => {

        let userValue = search.value; //прослущиваем событие клиента.
      let newArrFilter =  arr.filter((element) => element.keywords.includes(userValue));
        showCard(newArrFilter);//отображаем елементы по фильтру на строке выше.
    }
    search.addEventListener("input", () => emogySearch(data));// подписка на события по вводу.
